...
class CMS {
    ...
    private async processAllFile<T extends zc.$ZodObject>(
        ...
        return Promise.all(
            files
                ...
                .map((filename) =>
                    this.parseMarkdownFile(filename, metadataSchema),
                ),
        );
    }
    ...
    async init() {
        if (!isProd) {
            // In development, just load data directly.
            await this.loadData();
            return;
        }
            ...
            .then((data) => {
                this.posts = data.map(
                    (
                        ...
                        markdown_content: deserializeMarkdownContent(
